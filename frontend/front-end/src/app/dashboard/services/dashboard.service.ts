...
export class DashboardService {
  ...
  public addUser(user: User) {
    ...
    const nextId = currentList.length > 0 ? Math.max(...currentList.map(u => u.id)) + 1 : 1;
    user.id = nextId;
    this.users$.next([...currentList, user]);
  }
  ...
  public addRoute(route: Route) {
    const currentList = this.routes$.value;
    const nextId = currentList.length > 0 ? Math.max(...currentList.map(r => r.id)) + 1 : 1;
    route.id = nextId;
    this.routes$.next([...currentList, route]);
  }
  ...
  public addVehicle(vehicle: Vehicle) {
    const currentList = this.vehicles$.value;
    const nextId = currentList.length > 0 ? Math.max(...currentList.map(v => v.id)) + 1 : 1;
    vehicle.id = nextId;
    this.vehicles$.next([...currentList, vehicle]);
  }
  ...
  public addTrip(trip: Trip) {
    const currentList = this.trips$.value;
    const nextId = currentList.length > 0 ? Math.max(...currentList.map(t => t.id)) + 1 : 1;
    trip.id = nextId;
    this.trips$.next([...currentList, trip]);
  }
  ...
  public addDocument(doc: Document) {
    const currentList = this.documents$.value;
    const nextId = currentList.length > 0 ? Math.max(...currentList.map(d => d.id)) + 1 : 1;
    doc.id = nextId;
    this.documents$.next([doc, ...currentList]); // Add new uploads at the top
  }
